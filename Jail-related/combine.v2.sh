...
calculate_compensation() {
    # Динамически определяем имена файлов на основе ранее найденных блоков
    local FILE_BEFORE="snapshot_${SNAPSHOT_BLOCK_BEFORE}.csv"
    local FILE_AFTER="snapshot_${SNAPSHOT_BLOCK_JAIL}.csv"

    # Проверка, существуют ли входные файлы
    if [ ! -f "$FILE_BEFORE" ] || [ ! -f "$FILE_AFTER" ]; then
        echo "Ошибка: Один из файлов-снапшотов не найден! Расчет невозможен." >&2
        return 1
    fi
    ...
    echo ""
    echo "--- Фаза 6: Настройка и расчет компенсации ---"
    echo "========================================================"
    echo "Опция: Добавить дополнительную компенсацию сверх потерь?"
    ...
    local COMPENSATION_MULTIPLIER=1
    local OUTPUT_FILE="compensation_amounts.csv"
...
if [[ $? -eq 0 ]]; then
    ...
    if [[ $? -ne 0 ]]; then exit 1; fi # Выход, если первый снапшот не удался
    
    # --- Создаем второй снапшот (В ТЮРЬМЕ) ---
    create_single_snapshot "$SNAPSHOT_BLOCK_JAIL" "$WORKING_API"
    if [[ $? -ne 0 ]]; then exit 1; fi # Выход, если второй снапшот не удался
